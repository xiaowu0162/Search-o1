...
if __name__ == '__main__':
    ...
    # for shard in ['05']:
    in_file_name = f'/fsx-comem/diwu0162/NaturalThoughts/data/NaturalThoughts.jsonl.part{shard}'
    logs = [json.loads(line) for line in open(in_file_name).readlines()]
    out_file_name = f'logs_hint_distillation_naturalthoughts_shard_{shard}.jsonl'   # .new just to prevent being overwritten by old runs

    logs_filtered = []
    for entry in logs:
        for response_entry in entry['responses']:
            if response_entry['response_model'] == 'DeepSeek-R1':
                entry['response'] = response_entry['response']
                logs_filtered.append(entry)
                break
    logs = logs_filtered
    print(f'{len(logs)} entries with DeepSeek-R1 response after filtering')
