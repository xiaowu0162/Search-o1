...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Teacher-generated hint file."
    )
    return parser.parse_args()


def process_question_instruct_add_hint(task, instruction):
    if task in ['gpqa']:
...
def main():
    ...
    # load hints 
    hints = [json.loads(line) for line in open(args.hint_file).readlines()]
    question_to_hint = {x['question']: x['hint']['content']['hint'] for x in hints}
    question_to_hint = {k.split('Generate a correct Python program that passes all tests for the given problem. You should provide your final code within a Python code block using triple backticks (```python\nYOUR_CODE\n```).')[-1].lower().strip(): v for k, v in question_to_hint.items()}  # patch for livecode
    ...
    if model_short_name in ['qwq', 'ds-llama-8b', 'ds-qwen-7b', 'ds-qwen-32b', 'sky-t1']:
        if dataset_name in ['math500', 'gpqa', 'aime', 'amc', 'livecode']:
            output_dir = f'./outputs/{dataset_name}.{model_short_name}.directwithhint'
        else:
            output_dir = f'./outputs/runs.qa/{dataset_name}.{model_short_name}.directwithhint'
    else:
        output_dir = f'./outputs/runs.baselines/{dataset_name}.{model_short_name}.directwithhint'
    ...
    with open(data_path, mode='r', encoding='utf-8') as json_file:
        ...
        assert len(filtered_data) == len(hints)
    ...
    for item in filtered_data:
        ...
        user_prompt_processed = process_question_instruct_add_hint(dataset_name, user_prompt)
        ...
        hint = question_to_hint[extracted_q_lower]
